...
+ (void)requestType:(RequestType)type
    ...
    [manager requestWithType:type urlString:urlSrting prama:pramas success:success fail:fail];
...
- (void)requestWithType:(RequestType)type urlString:(NSString *)urlString prama:(NSDictionary *)pramas success:(RequestSuccessed)success fail:(RequestFailed)fail {
   ...
   //1、使用urlSession
    ...
    if (type == POST) {
        ...
        //post参数需要加紧body里面，将字典中的内容转成符合 key = value&key=value的形式
        if (pramas.count > 0) {
